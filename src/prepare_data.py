...
new_ct_dir = f'../Lung_GTV_sz{size}_st{stride}_ds{down_scale}_st{slice_thickness}'
...
for ct_file in os.listdir(ct_dir):
    ...
    # 将CT和金标准入读内存
    ct = sitk.ReadImage(os.path.join(ct_dir, ct_file, 'data.nii.gz'), sitk.sitkInt16)
    ct_array = sitk.GetArrayFromImage(ct)

    seg = sitk.ReadImage(os.path.join(ct_dir, ct_file, 'label.nii.gz'), sitk.sitkInt8)
    ...
    while end_slice <= ct_array.shape[0] - 1:
        ...
        new_ct_name = 'data-' + str(file_index) + '.nii.gz'
        new_seg_name = 'label-' + str(file_index) + '.nii.gz'

        os.makedirs(os.path.join(new_ct_dir, ct_file), exist_ok=True)

        sitk.WriteImage(new_ct, os.path.join(new_ct_dir, ct_file, new_ct_name))
        sitk.WriteImage(new_seg, os.path.join(new_ct_dir, ct_file, new_seg_name))
